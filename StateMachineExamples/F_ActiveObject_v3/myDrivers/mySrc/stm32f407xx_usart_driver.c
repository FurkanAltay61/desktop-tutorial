	...
	for(uint32_t i=0;i < Len ; i++){
		...
		if(pUSARTHandle->USART_Config.WordLength == USART_WORDLEN_9BITS){
			...
			if(pUSARTHandle->USART_Config.ParityControl == USART_PARITY_DISABLE){
				...
				//read only first 8 bits so mask the DR with 0xFF
				*pRxBuffer = (pUSARTHandle->pUSARTx->DR & (uint8_t)0xFF);
				...
				//read only first 8 bits so mask the DR with 0x7F
...
uint8_t USART_SendDataIT(USART_Handle_t *pUSARTHandle,uint8_t *pTxBuffer, uint32_t Len){
	...
	if(txstate != USART_BUSY_IN_TX){
		...
		//Impelemtn the code to enable interrupt for TXE
		pUSARTHandle->pUSARTx->CR1 |= ( 1 << USART_CR1_TXEIE);

		//Impelemtn the code to enable interrupt for TC
...
uint8_t USART_ReceiveDataIT(USART_Handle_t *pUSARTHandle, uint8_t *pRxBuffer, uint32_t Len){
	...
	if(rxstate != USART_BUSY_IN_RX){
		...
		//Impelemtn the code to enable interrupt for RXNE
...
//		uint32_t dummy_data;
//		dummy_data = pUSARTHandle->pUSARTx->SR;
//		dummy_data = pUSARTHandle->pUSARTx->DR;
